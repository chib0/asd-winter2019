...
def test_server_threaded(monkeypatch):
    ...
    monkeypatch.setattr(server.dispatchers.repository.DispatcherRepository, 'get_dispatcher', dispatcher_mock)
...
def test_server_non_threaded(monkeypatch, mock_publish, mock_encoder):
    ...
    monkeypatch.setattr(server.dispatchers.repository.DispatcherRepository, 'get_dispatcher', dispatcher_mock)
    kwargs = dict(host="127.0.0.1", port=1234)
    server._run_server(publish=mock_publish, encoder=mock_encoder, **kwargs, run_threaded=True)
    mock_server = get_server_mock.return_value
